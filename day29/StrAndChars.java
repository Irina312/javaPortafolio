...
public class StrAndChars {
	public static void main(String[] args) {
		...
		for (int i = 0; i < str.length(); i++) {
			...
			// for each loop -> to print each element one by one 
			//  another words for each loop iterates over the array be getting each element
			for (char ch : chArr) {
			...
			System.out.println(chArr2[5]); // D -> to print one element
		}
	}
